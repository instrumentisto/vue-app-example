...
const tests = {
  ...
  'Authorization with valid credentials': (client) => {
    ...
    loginPage.navigate()
      ...
      .assert.containsText(profilePage.section.main.elements.userName.selector, 'Test User')
      .assert.containsText(profilePage.section.main.elements.userEmail.selector, 'test@gmail.com');
  ...
  'Authorization with wrong credentials': (client) => {
    ...
    loginPage.navigate()
      ...
      .assert.containsText('@error', client.globals.localeData.errors.access_denied);
